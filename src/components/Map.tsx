...
function Map() {
	...
	const handleMouseDown = (e: any) => {
		...
		isDragging = true
		setInitialX(clientX - offsetX)
		setInitialY(clientY - offsetY)
	}
  ...
  const handleMouseMove = (e: any) => {
    if (isDragging) {
      e.preventDefault();
      const { clientX, clientY } = e
      setOffsetX(clientX - initialX)
      setOffsetY(clientY - initialY)
    }
  }
	...
	return (
		<>
			<div className="relative overflow-hidden max-w-full max-h-full">
				<div className={`flex flex-wrap translate-x-[${offsetX}px]`}>
					<div>
						<video
